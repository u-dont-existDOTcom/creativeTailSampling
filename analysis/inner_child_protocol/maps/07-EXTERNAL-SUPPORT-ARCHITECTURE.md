...
flowchart TD
    ...
    C --> F["Receive → observe → participate → initiate"]
    D --> F
    E --> F
    F --> G["Do one piece without helper"]
    G --> H{"Function becoming internally accessible?"}
    H -- "not yet" --> I["Continue bounded borrowing / change exemplar / add support"]
    I --> F
    H -- "yes" --> J["Internalize function"]
    J --> K["Healthy interdependence may remain"]
    K --> L{"Severe stress temporarily blocks access?"}
